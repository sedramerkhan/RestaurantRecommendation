...
class GMap():
    def __init__(self, master_gui, cuisines, restaurants, height, width, my_location=ITE_POS):
        ...
        self.map_widget.set_position(*my_location)
        self.map_widget.set_zoom(14)
        self.my_loc_marker = self.add_marker(my_location, 'my location')
    ...
    def define_commands(self):
        def left_click_event(coordinates):
            ...
            distance = self.cal_distance(self.my_location, (min_res['lat'], min_res['lon']))
        ...
        def right_click_event(coordinates):
            self.my_location = coordinates
            self.my_loc_marker.delete()
            self.my_loc_marker = self.add_marker(self.my_location, 'my location')

            
        self.map_widget.add_left_click_map_command(left_click_event)
        self.map_widget.add_right_click_menu_command(
            label="set my location",
            command=right_click_event,
            pass_coords=True
        )
    ...
    def add_marker(self, position, text):
        return self.map_widget.set_marker(*position, text=text)
